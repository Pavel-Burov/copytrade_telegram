    ...
    async with TelegramClient(phone_number, api_id, api_hash) as client:
        ...
        async def new_message_handler(event):
            ...
            if 'message' in message and message['message']:
                ...
                prompt = (f"исходя из текста выведи формулу:company ticker=Buy or Sell, без лишних слов, твой ответ должен сосотоять только из формулы. Текст: {message["message"]}".replace("\r", " ")).replace("\n"," ")
...
# СКРИПТ ДЛЯ ОБРАБОТКИ СООБЩЕНИЙ В КАНАЛЕ